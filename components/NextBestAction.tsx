    ...
    return (
        <div className="space-y-8 animate-fade-in">
            <div className="text-center p-6 bg-slate-800/50 border border-slate-700 rounded-xl">
                ...
                <p className="text-lg text-slate-300 mt-2">Taking the next step — so you don’t have to.</p>
            ...
            <div className="bg-slate-800 border border-slate-700 rounded-xl p-6">
                <h3 className="text-lg font-bold text-slate-100 mb-4 text-center">Recent Autonomous Activity</h3>
                <div className="space-y-3 text-slate-300 max-w-lg mx-auto">
                    <div className="flex items-center gap-4 bg-slate-700/50 p-3 rounded-md">
            ...
            <div className="bg-slate-800 border border-slate-700 rounded-xl p-6">
                ...
                <p className="text-sm text-slate-300">{data.scenario}</p>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-8">
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-xl p-6">
                    ...
                    <div className="space-y-3 text-sm text-slate-300">
                        <p><strong className="text-yellow-300">Why it’s the right next step:</strong> {data.justification.why}</p>
                        <p><strong className="text-yellow-300">Data used to decide:</strong> {data.justification.data}</p>
                        <p><strong className="text-yellow-300">Logic:</strong> {data.justification.logic}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-slate-800 border-2 border-green-600/50 rounded-xl shadow-lg">
                <div className="p-6">
                    <h3 className="text-lg font-bold text-green-400 mb-4"><i className="fa-solid fa-rocket mr-3"></i>{data.executedAction.title}</h3>
                    <div className="bg-slate-900/50 p-4 rounded-md border border-slate-700">
                        <p className="text-sm font-semibold text-slate-100 mb-2">{data.executedAction.post.platform}</p>
                        <p className="text-sm text-slate-300 whitespace-pre-wrap leading-relaxed">{data.executedAction.post.caption}</p>
                        <p className="text-cyan-300 text-xs italic mt-4">{data.executedAction.post.hashtags}</p>
                    </div>
            ...
            <div className="mt-12 pt-8 border-t-2 border-dashed border-slate-700">
                ...
                <div className="grid md:grid-cols-2 gap-6 mt-8 text-sm">
                    {data.upgradeTriggers.map((trigger, index) => (
                        <div key={index} className="bg-slate-800 border border-slate-700 rounded-xl p-5">
                            ...
                            <p className="text-slate-300 mb-4">{trigger.condition}</p>
                            <p className="font-bold text-purple-300 mb-2">✨ "{trigger.message}"</p>
                            <div className="text-slate-400 space-y-3">
                                <p><strong className="text-slate-200">Value:</strong> {trigger.value}</p>
                                <p><strong className="text-slate-200">Timing:</strong> {trigger.timing}</p>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                <div className="text-center mt-8 pt-6 border-t border-slate-800">
                    <p className="text-sm text-yellow-300/80 italic"><span className="font-bold">💡 Product Insight:</span> {data.productInsight}</p>
                </div>
            </div>
        </div>
    );
