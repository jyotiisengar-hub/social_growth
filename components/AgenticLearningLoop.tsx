...
const SummaryCard: React.FC<{ title: string; children: React.ReactNode; icon: string; borderColor: string; }> = ({ title, children, icon, borderColor }) => (
    <div className={`flex-1 bg-slate-50 border ${borderColor} rounded-xl p-6`}>
...
export const AgenticLearningLoop: React.FC<AgenticLearningLoopProps> = ({ socialPlan }) => {
    ...
    return (
        <div className="text-center p-6 bg-white border border-slate-200 rounded-xl animate-fade-in">
            <h2 className="text-2xl font-bold text-indigo-600 mb-2">
                Analyzing last week’s engagement to refine next week’s growth plan…
            </h2>
            <p className="text-slate-600 text-sm max-w-2xl mx-auto mb-8 relative group">
            ...
            <div className="flex flex-col md:flex-row gap-6 mb-8 text-left">
                ...
                <SummaryCard title="Last Week’s Performance Summary" icon="fa-solid fa-chart-line" borderColor="border-slate-300">
                    <p className="text-sm text-slate-600 mb-4">You provided feedback on the last plan, here's what the AI is learning from:</p>
                    <div className="space-y-3">
                        {usefulPosts.length > 0 && (
                             <div>
                                <p className="text-sm font-semibold text-green-600 mb-2 flex items-center gap-2"><i className="fa-solid fa-thumbs-up"></i>What Worked Well:</p>
                        ...
                        {notUsefulPosts.length > 0 && (
                            <div>
                                <p className="text-sm font-semibold text-red-600 mb-2 flex items-center gap-2"><i className="fa-solid fa-thumbs-down"></i>What to Improve:</p>
                                <ul className="list-disc list-inside text-sm text-slate-700 space-y-1">
                ...
                <SummaryCard title="Next Week’s Smart Plan" icon="fa-solid fa-wand-magic-sparkles" borderColor="border-purple-300">
                    <div className="flex flex-col h-full">
                        <p className="text-sm text-slate-600 mb-4">The AI is now crafting a new plan based on these insights...</p>
                        <div className="flex-grow flex items-center justify-center">
                            <div className="text-center">
                                ...
                                <p className="text-sm font-semibold text-purple-600">Building on your successes...</p>
